...
func TestMountpointDoesNotExist(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "does not exist") {
		t.Fatalf("wrong error from mount: %v", err)
	}
}
...
func TestStatfs(t *testing.T) {
...

	{
		...
		// TODO(irfansharif): This was uint32(1000) prior and failed, investigate.
		if g, e := st.Bsize, uint32(128); g != e {
...
func TestCreate(t *testing.T) {
	...
	if runtime.GOOS == "darwin" {
		...
		// TODO(irfansharif): For some reason this flag appeared in OS X. Investigate.
		want.Flags |= fuse.OpenExclusive
	...
	if got != want {
		t.Fatalf("create saw %+v, want %+v", got, want)
	}
