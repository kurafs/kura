...
func TestFilePersistence(t *testing.T) {
	...
	waitStorage, shutdownStorage, err := storageserver.Start(logger, 10669, "127.0.0.1", tdir)
	if err != nil {
		t.Fatal(err)
	}

	waitMetadata, shutdownMetadata, err := metadataserver.Start(logger, 10670, "127.0.0.1", "localhost:10669")
	if err != nil {
		t.Fatal(err)
	}
